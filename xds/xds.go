...
var cs = CacheEventHandler{
	Notifier: &ch,
}

func InitializeCache() *grpcServer {
...
func StartXDS(s *grpcServer) {

	lis, err := net.Listen("tcp", bootstrap.Iface+":"+bootstrap.Mgmtport)
	if err != nil {
		log.Print(err)
	}
	...
	log.Printf("XDS listening on %v:%v", bootstrap.Iface, bootstrap.Mgmtport)
...
func (s *grpcServer) IncrementalAggregatedResources(_ discovery.AggregatedDiscoveryService_IncrementalAggregatedResourcesServer) error {
	return errors.New("not implemented")
}

func (s *grpcServer) StreamAggregatedResources(stream discovery.AggregatedDiscoveryService_StreamAggregatedResourcesServer) error {
...
func (xh *xdsHandler) stream(st grpcStream) (err error) {
	...
	// for {
	// 	req, _ := st.Recv()
	// 	log.Printf("Stream received: %+s", req.TypeUrl)
	// 	// r, ok := xh.resources[req.TypeUrl]
	// 	// if !ok {
	...
	for {
		...
		log.Printf("Resource stream request: %v", r)
